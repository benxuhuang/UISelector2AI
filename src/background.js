...
async function transcribeAudioEndpoint(audioBase64, sttCfg) {
  ...
  if (!audioBlob.size) throw new Error('Audio vacío (0 bytes)');
...
async function transcribeChatCompletions(audioBase64, sttCfg) {
  // OpenRouter espera WAV en input_audio. Convertimos WebM → WAV vía offscreen.
...
async function stopAndProcess() {
  ...
  if (cfg.llm?.enabled && cfg.llm.apiKey && text) {
    try {
      ...
      console.warn('[voice] LLM refine falló, devolviendo raw:', e.message);
    }
  }
  return text;
}
